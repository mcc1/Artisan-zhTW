...
public class ExpertSolverSettings
{
    ...
    public bool Draw()
    {
        ...
        if (ImGui.CollapsingHeader("起手設定"))
        {
            ...
            changed |= ImGui.Checkbox($"如果在使用 [{Skills.MuscleMemory.NameOfAction()}] 後处于 [{Condition.Good.ToLocalizedString()}] {ConditionString}，使用 [{Skills.IntensiveSynthesis.NameOfAction()}]（400%）而不是 [{Skills.RapidSynthesis.NameOfAction()}]（500%）", ref MuMeIntensiveGood);
            ...
            changed |= ImGui.Checkbox($"如果在 [{Skills.MuscleMemory.NameOfAction()}] 的最後一步時不是 [{Condition.Centered.ToLocalizedString()}] {ConditionString}，使用 [{Skills.IntensiveSynthesis.NameOfAction()}]（如果可以，强制使用 [{Skills.HeartAndSoul.NameOfAction()}] ）", ref MuMeIntensiveLastResort);
            changed |= ImGui.Checkbox($"如果 [{Skills.Veneration.NameOfAction()}] 还处于激活状态，在 [{Condition.Primed.ToLocalizedString()}] {ConditionString}時使用 [{Skills.Manipulation.NameOfAction()}] 。", ref MuMePrimedManip);
            changed |= ImGui.Checkbox($"在出现不利状态時使用 [{Skills.Observe.NameOfAction()}] ，而不是使用 [{Skills.RapidSynthesis.NameOfAction()}] 来消耗{DurabilityString}。", ref MuMeAllowObserve);
            ImGui.Text($"僅当 [{Skills.MuscleMemory.NameOfAction()}] 的剩余步骤超过此数量時，才允许使用 [{Skills.Manipulation.NameOfAction()}] 。");
            ImGui.PushItemWidth(250);
            changed |= ImGui.SliderInt("###MumeMinStepsForManip", ref MuMeMinStepsForManip, 0, 5);
            ImGui.Text($"僅当 [{Skills.MuscleMemory.NameOfAction()}] 的剩余步骤超过此数量時，才允许使用 [{Skills.Veneration.NameOfAction()}] 。");
            ImGui.PushItemWidth(250);
        ...
        if (ImGui.CollapsingHeader("主循環設定"))
        {
            ...
            changed |= ImGui.Checkbox($"低{DurabilityString}時，优先選擇 [{Skills.Observe.NameOfAction()}] 而不是非-[{Condition.Pliant.ToLocalizedString()}] [{Skills.Manipulation.NameOfAction()}]，在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之前", ref MidBaitPliantWithObservePreQuality);
            changed |= ImGui.Checkbox($"低{DurabilityString}時，优先選擇 [{Skills.Observe.NameOfAction()}] 而不是非-[{Condition.Pliant.ToLocalizedString()}] [{Skills.Manipulation.NameOfAction()}] / [{Skills.Innovation.NameOfAction()}]+[{Skills.TrainedFinesse.NameOfAction()}]，在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之後", ref MidBaitPliantWithObserveAfterIQ);
            changed |= ImGui.Checkbox($"在 [{Condition.Primed.ToLocalizedString()}] {ConditionString} 下使用 [{Skills.Manipulation.NameOfAction()}]，在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之前", ref MidPrimedManipPreQuality);
            changed |= ImGui.Checkbox($"在 [{Condition.Primed.ToLocalizedString()}] {ConditionString} 下使用 [{Skills.Manipulation.NameOfAction()}]，在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之後，如果有足够的 CP 以良好利用 {DurabilityString}", ref MidPrimedManipAfterIQ);
            changed |= ImGui.Checkbox($"在不利的 {ConditionString} 下允许使用 [{Skills.Observe.NameOfAction()}]，沒有增益效果", ref MidKeepHighDuraUnbuffed);
            ...
            changed |= ImGui.Checkbox($"如果我们在 [{Condition.GoodOmen.ToLocalizedString()}] 上還有大 {ProgressString} 赤字（超过 [{Skills.IntensiveSynthesis.NameOfAction()}] 可以完成的），则允许使用 [{Skills.Veneration.NameOfAction()}]", ref MidAllowVenerationGoodOmen);
            changed |= ImGui.Checkbox($"在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之後，如果我们在 [{Condition.GoodOmen.ToLocalizedString()}] 上還有大 {ProgressString} 赤字（超过 [{Skills.RapidSynthesis.NameOfAction()}] 可以完成的），则允许使用 [{Skills.Veneration.NameOfAction()}]", ref MidAllowVenerationAfterIQ);
            changed |= ImGui.Checkbox($"在沒有增益效果的情况下，花费 [{Condition.Good.ToLocalizedString()}] {ConditionString} 用于 [{Skills.IntensiveSynthesis.NameOfAction()}]，如果我们需要更多 {ProgressString}", ref MidAllowIntensiveUnbuffed);
            ...
            changed |= ImGui.Checkbox($"如果我们需要更多 {Buffs.InnerQuiet.NameOfBuff()} 層，花费 [{Condition.Good.ToLocalizedString()}] {ConditionString} 用于 [{Skills.PreciseTouch.NameOfAction()}]", ref MidAllowPrecise);
            changed |= ImGui.Checkbox($"考虑使用 [{Condition.Sturdy.ToLocalizedString()}] {ConditionString} 的 [{Skills.HeartAndSoul.NameOfAction()}] + [{Skills.PreciseTouch.NameOfAction()}]，作為积累 {Buffs.InnerQuiet.NameOfBuff()} 層的良好選擇", ref MidAllowSturdyPreсise);
            changed |= ImGui.Checkbox($"考虑使用 [{Condition.Centered.ToLocalizedString()}] {ConditionString} 的 [{Skills.HastyTouch.NameOfAction()}]，作為积累 {Buffs.InnerQuiet.NameOfBuff()} 層的良好選擇（85% 成功，10 {DurabilityString}）", ref MidAllowCenteredHasty);
            changed |= ImGui.Checkbox($"考虑使用 [{Condition.Sturdy.ToLocalizedString()}] {ConditionString} 的 [{Skills.HastyTouch.NameOfAction()}]，作為积累 {Buffs.InnerQuiet.NameOfBuff()} 層的良好選擇（50% 成功，5 {DurabilityString}）", ref MidAllowSturdyHasty);
            ...
            changed |= ImGui.Checkbox($"在 [{Condition.GoodOmen.ToLocalizedString()}] {ConditionString} 下使用 [{Skills.Observe.NameOfAction()}]，如果我们本来會在 [{Condition.Good.ToLocalizedString()}] {ConditionString} 上使用 [{Skills.TricksOfTrade.NameOfAction()}]", ref MidObserveGoodOmenForTricks);
        }
        ImGui.Unindent();
        ...
        changed |= ImGui.Checkbox($"终结技：使用 {Skills.CarefulObservation.NameOfAction()} 為 {Condition.Good.ToLocalizedString()} {ConditionString} 争取一下 {Skills.ByregotsBlessing.NameOfAction()}", ref FinisherBaitGoodByregot);
        changed |= ImGui.Checkbox($"紧急情况：如果制作力不够用了，使用 {Skills.CarefulObservation.NameOfAction()} 為 [{Condition.Good.ToLocalizedString()} {ConditionString} 争取一下 {Skills.TricksOfTrade.NameOfAction()}", ref EmergencyCPBaitGood);
