...
public class ExpertSolverSettings
{
    ...
    public bool Draw()
    {
        ...
        if (ImGui.CollapsingHeader("主循環設定"))
        {
            ...
            changed |= ImGui.Checkbox($"低{DurabilityString}時，優先選擇 [{Skills.Observe.NameOfAction()}] 而不是非-[{Condition.Pliant.ToLocalizedString()}] [{Skills.Manipulation.NameOfAction()}]，在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之前", ref MidBaitPliantWithObservePreQuality);
            changed |= ImGui.Checkbox($"低{DurabilityString}時，優先選擇 [{Skills.Observe.NameOfAction()}] 而不是非-[{Condition.Pliant.ToLocalizedString()}] [{Skills.Manipulation.NameOfAction()}] / [{Skills.Innovation.NameOfAction()}]+[{Skills.TrainedFinesse.NameOfAction()}]，在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之後", ref MidBaitPliantWithObserveAfterIQ);
            ...
            changed |= ImGui.Checkbox($"在 [{Condition.Primed.ToLocalizedString()}] {ConditionString} 下使用 [{Skills.Manipulation.NameOfAction()}]，在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之後，如果有足夠的 CP 以良好利用 {DurabilityString}", ref MidPrimedManipAfterIQ);
            changed |= ImGui.Checkbox($"在不利的 {ConditionString} 下允許使用 [{Skills.Observe.NameOfAction()}]，沒有增益效果", ref MidKeepHighDuraUnbuffed);
            changed |= ImGui.Checkbox($"在不利的 {ConditionString} 下，在 {Buffs.Veneration.NameOfBuff()} 下允許使用 [{Skills.Observe.NameOfAction()}]", ref MidKeepHighDuraVeneration);
            changed |= ImGui.Checkbox($"如果我們在 [{Condition.GoodOmen.ToLocalizedString()}] 上還有大 {ProgressString} 赤字（超過 [{Skills.IntensiveSynthesis.NameOfAction()}] 可以完成的），則允許使用 [{Skills.Veneration.NameOfAction()}]", ref MidAllowVenerationGoodOmen);
            changed |= ImGui.Checkbox($"在 {Buffs.InnerQuiet.NameOfBuff()} 有 10 層之後，如果我們在 [{Condition.GoodOmen.ToLocalizedString()}] 上還有大 {ProgressString} 赤字（超過 [{Skills.RapidSynthesis.NameOfAction()}] 可以完成的），則允許使用 [{Skills.Veneration.NameOfAction()}]", ref MidAllowVenerationAfterIQ);
            changed |= ImGui.Checkbox($"在沒有增益效果的情況下，花費 [{Condition.Good.ToLocalizedString()}] {ConditionString} 用於 [{Skills.IntensiveSynthesis.NameOfAction()}]，如果我們需要更多 {ProgressString}", ref MidAllowIntensiveUnbuffed);
            changed |= ImGui.Checkbox($"在 [{Skills.Veneration.NameOfAction()}] 下，花費 [{Condition.Good.ToLocalizedString()}] {ConditionString} 用於 [{Skills.IntensiveSynthesis.NameOfAction()}]，如果我們需要更多 {ProgressString}", ref MidAllowIntensiveVeneration);
            changed |= ImGui.Checkbox($"如果我們需要更多 {Buffs.InnerQuiet.NameOfBuff()} 層，花費 [{Condition.Good.ToLocalizedString()}] {ConditionString} 用於 [{Skills.PreciseTouch.NameOfAction()}]", ref MidAllowPrecise);
            changed |= ImGui.Checkbox($"考慮使用 [{Condition.Sturdy.ToLocalizedString()}] {ConditionString} 的 [{Skills.HeartAndSoul.NameOfAction()}] + [{Skills.PreciseTouch.NameOfAction()}]，作為積累 {Buffs.InnerQuiet.NameOfBuff()} 層的良好選擇", ref MidAllowSturdyPreсise);
            changed |= ImGui.Checkbox($"考慮使用 [{Condition.Centered.ToLocalizedString()}] {ConditionString} 的 [{Skills.HastyTouch.NameOfAction()}]，作為積累 {Buffs.InnerQuiet.NameOfBuff()} 層的良好選擇（85% 成功，10 {DurabilityString}）", ref MidAllowCenteredHasty);
            changed |= ImGui.Checkbox($"考慮使用 [{Condition.Sturdy.ToLocalizedString()}] {ConditionString} 的 [{Skills.HastyTouch.NameOfAction()}]，作為積累 {Buffs.InnerQuiet.NameOfBuff()} 層的良好選擇（50% 成功，5 {DurabilityString}）", ref MidAllowSturdyHasty);
            changed |= ImGui.Checkbox($"在 [{Condition.Good.ToLocalizedString()}] {ConditionString} + {Buffs.Innovation.NameOfBuff()} + {Buffs.GreatStrides.NameOfBuff()} 的情況下，考慮使用 [{Skills.PreparatoryTouch.NameOfAction()}]，假設我們有足夠的 {DurabilityString}", ref MidAllowGoodPrep);
            changed |= ImGui.Checkbox($"在 [{Condition.Sturdy.ToLocalizedString()}] {ConditionString} + {Buffs.Innovation.NameOfBuff()} 的情況下，考慮使用 [{Skills.PreparatoryTouch.NameOfAction()}]，假設我們有足夠的 {DurabilityString}", ref MidAllowSturdyPrep);
            changed |= ImGui.Checkbox($"在 [{Skills.Innovation.NameOfAction()}] + {QualityString} 組合之前使用 [{Skills.GreatStrides.NameOfAction()}]", ref MidGSBeforeInno);
            changed |= ImGui.Checkbox($"在開始 {QualityString} 階段之前完成 {ProgressString}", ref MidFinishProgressBeforeQuality);
            changed |= ImGui.Checkbox($"在 [{Condition.GoodOmen.ToLocalizedString()}] {ConditionString} 下使用 [{Skills.Observe.NameOfAction()}]，如果我們本來會在 [{Condition.Good.ToLocalizedString()}] {ConditionString} 上使用 [{Skills.TricksOfTrade.NameOfAction()}]", ref MidObserveGoodOmenForTricks);
        }
        ImGui.Unindent();
        ...
        changed |= ImGui.Checkbox($"終結技：使用 {Skills.CarefulObservation.NameOfAction()} 為 {Condition.Good.ToLocalizedString()} {ConditionString} 爭取一下 {Skills.ByregotsBlessing.NameOfAction()}", ref FinisherBaitGoodByregot);
        changed |= ImGui.Checkbox($"緊急情況：如果製作力不夠用了，使用 {Skills.CarefulObservation.NameOfAction()} 為 [{Condition.Good.ToLocalizedString()} {ConditionString} 爭取一下 {Skills.TricksOfTrade.NameOfAction()}", ref EmergencyCPBaitGood);
